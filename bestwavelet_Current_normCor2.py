...
for win in windowlist:
	...
	for a in range(1,9):
		...
		for b in range(5):
			...
			for aa in range(6):
				train_data_lstm, train_label_lstm = shuffle(train_data_lstm, train_label_lstm)


			model = buildManyToOneModel_model9(train_data_lstm.shape)

			model.fit(train_data_lstm, train_label_lstm, epochs = 225, batch_size = 12, validation_split = 0.2, callbacks = [callback])
